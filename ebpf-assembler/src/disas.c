...
int main(int argc, char **argv) {
  ...
  for (int i=7; i<size; i+=8) {
    ...
    */
    ...
    // div dst src
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_DIV) | BPF_X) && off == 0)
      printf("div r%d r%d", dst_reg, src_reg);
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_DIV) | BPF_X) && off == 1)
      printf("sdiv r%d r%d", dst_reg, src_reg);
    // or dst src
    ...
    // mod dst src
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_MOD) | BPF_X) && off == 0)
      printf("mod r%d r%d", dst_reg, src_reg);
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_MOD) | BPF_X) && off == 1)
      printf("smod r%d r%d", dst_reg, src_reg);
    // xor dst src
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_XOR) | BPF_X))
      printf("xor r%d r%d", dst_reg, src_reg);
    // mov dst src
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_MOV) | BPF_X) && off == 0)
      printf("mov r%d r%d", dst_reg, src_reg);
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_MOV) | BPF_X) && off == 8)
      printf("movsx8 r%d r%d", dst_reg, src_reg);
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_MOV) | BPF_X) && off == 16)
      printf("movsx16 r%d r%d", dst_reg, src_reg);
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_MOV) | BPF_X) && off == 32)
      printf("movsx32 r%d r%d", dst_reg, src_reg);
    // arsh dst src
    ...
    */
    ...
    // div dst imm
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_DIV) | BPF_K) && off == 0)
      printf("div r%d %d", dst_reg, imm);
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_DIV) | BPF_K) && off == 1)
      printf("sdiv r%d %d", dst_reg, imm);
    // or dst imm
    ...
    // mod dst imm
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_MOD) | BPF_K) && off == 0)
      printf("mod r%d %d", dst_reg, imm);
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_MOD) | BPF_K) && off == 1)
      printf("smod r%d %d", dst_reg, imm);
    // xor dst imm
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_XOR) | BPF_K))
      printf("xor r%d %d", dst_reg, imm);
    // mov dst imm
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_MOV) | BPF_K) && off == 0)
      printf("mov r%d %d", dst_reg, imm);
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_MOV) | BPF_K) && off == 8)
      printf("movsx8 r%d r%d", dst_reg, imm);
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_MOV) | BPF_K) && off == 16)
      printf("movsx16 r%d r%d", dst_reg, imm);
    else if (opcode == (BPF_ALU64 | BPF_OP(BPF_MOV) | BPF_K) && off == 32)
      printf("movsx32 r%d r%d", dst_reg, imm);
    // arsh dst imm
    ...
    */
    ...
    // div32 dst src
    else if (opcode == (BPF_ALU | BPF_OP(BPF_DIV) | BPF_X) && off == 0)
      printf("div32 r%d r%d", dst_reg, src_reg);
    else if (opcode == (BPF_ALU | BPF_OP(BPF_DIV) | BPF_X) && off == 1)
      printf("sdiv32 r%d r%d", dst_reg, src_reg);
    // or32 dst src
    ...
    // mod32 dst src
    else if (opcode == (BPF_ALU | BPF_OP(BPF_MOD) | BPF_X) && off == 0)
      printf("mod32 r%d r%d", dst_reg, src_reg);
    else if (opcode == (BPF_ALU | BPF_OP(BPF_MOD) | BPF_X) && off == 1)
      printf("smod32 r%d r%d", dst_reg, src_reg);
    // xor32 dst src
    else if (opcode == (BPF_ALU | BPF_OP(BPF_XOR) | BPF_X))
      printf("xor32 r%d r%d", dst_reg, src_reg);
    // mov32 dst src
    else if (opcode == (BPF_ALU | BPF_OP(BPF_MOV) | BPF_X) && off == 0)
      printf("mov32 r%d r%d", dst_reg, src_reg);
    else if (opcode == (BPF_ALU | BPF_OP(BPF_MOV) | BPF_X) && off == 8)
      printf("mov32sx8 r%d r%d", dst_reg, src_reg);
    else if (opcode == (BPF_ALU | BPF_OP(BPF_MOV) | BPF_X) && off == 16)
      printf("mov32sx16 r%d r%d", dst_reg, src_reg);
    // arsh32 dst src
    ...
    */
    ...
    // div dst imm
    else if (opcode == (BPF_ALU | BPF_OP(BPF_DIV) | BPF_K) && off == 0)
      printf("div32 r%d %d", dst_reg, imm);
    else if (opcode == (BPF_ALU | BPF_OP(BPF_DIV) | BPF_K) && off == 1)
      printf("sdiv32 r%d %d", dst_reg, imm);
    // or dst imm
    ...
    // mod dst imm
    else if (opcode == (BPF_ALU | BPF_OP(BPF_MOD) | BPF_K) && off == 0)
      printf("mod32 r%d %d", dst_reg, imm);
    else if (opcode == (BPF_ALU | BPF_OP(BPF_MOD) | BPF_K) && off == 1)
      printf("smod32 r%d %d", dst_reg, imm);
    // xor dst imm
    else if (opcode == (BPF_ALU | BPF_OP(BPF_XOR) | BPF_K))
      printf("xor32 r%d %d", dst_reg, imm);
    // mov dst imm
    else if (opcode == (BPF_ALU | BPF_OP(BPF_MOV) | BPF_K) && off == 0)
      printf("mov32 r%d %d", dst_reg, imm);
    else if (opcode == (BPF_ALU | BPF_OP(BPF_MOV) | BPF_K) && off == 8)
      printf("mov32sx8 r%d r%d", dst_reg, imm);
    else if (opcode == (BPF_ALU | BPF_OP(BPF_MOV) | BPF_K) && off == 16)
      printf("mov32sx16 r%d r%d", dst_reg, imm);
    // arsh dst imm
    ...
    */
    ...
    // bswap dst
    else if (opcode == (BPF_ALU64 | BPF_END))
      printf("bswap%d r%d", imm, dst_reg);
    ...
    */
    ...
    // addx64 dst src off
    else if (opcode == (BPF_STX | BPF_SIZE(0x18) | BPF_ATOMIC) &&
             imm == BPF_ADD)
    ...
    */
    ...
    else if (opcode == (BPF_LD | BPF_SIZE(0x18) | BPF_ABS))
    ...
    else if (opcode == (BPF_LD | BPF_SIZE(0x18) | BPF_IND))
      printf("ldind64 r%d %d", src_reg, imm);

    /* BPF_LDX_MEM */
    else if (opcode == (BPF_LDX | BPF_SIZE_8 | BPF_MEM))
      printf("ldx8 r%d r%d %d", dst_reg, src_reg, off);
    else if (opcode == (BPF_LDX | BPF_SIZE_16 | BPF_MEM))
      printf("ldx16 r%d r%d %d", dst_reg, src_reg, off);
    else if (opcode == (BPF_LDX | BPF_SIZE_32 | BPF_MEM))
      printf("ldx32 r%d r%d %d", dst_reg, src_reg, off);
    else if (opcode == (BPF_LDX | BPF_SIZE_64 | BPF_MEM))
      printf("ldx64 r%d r%d %d", dst_reg, src_reg, off);

    /* BPF_LDX_MEMSX */
    else if (opcode == (BPF_LDX | BPF_SIZE_8 | BPF_MEMSX))
      printf("ldxs8 r%d r%d %d", dst_reg, src_reg, off);
    else if (opcode == (BPF_LDX | BPF_SIZE_16 | BPF_MEMSX))
      printf("ldxs16 r%d r%d %d", dst_reg, src_reg, off);
    else if (opcode == (BPF_LDX | BPF_SIZE_32 | BPF_MEMSX))
      printf("ldxs32 r%d r%d %d", dst_reg, src_reg, off);

    /* BPF_ST_MEM */
    else if (opcode == (BPF_ST | BPF_SIZE_8 | BPF_MEM))
      printf("st8 r%d %d %d", dst_reg, off, imm);
    else if (opcode == (BPF_ST | BPF_SIZE_16 | BPF_MEM))
      printf("st16 r%d %d %d", dst_reg, off, imm);
    else if (opcode == (BPF_ST | BPF_SIZE_32 | BPF_MEM))
      printf("st32 r%d %d %d", dst_reg, off, imm);
    else if (opcode == (BPF_ST | BPF_SIZE_64 | BPF_MEM))
      printf("st64 r%d %d %d", dst_reg, off, imm);

    /* BPF_STX_MEM */
    else if (opcode == (BPF_STX | BPF_SIZE_8 | BPF_MEM))
      printf("stx8 r%d r%d %d", dst_reg, src_reg, off);
    else if (opcode == (BPF_STX | BPF_SIZE_16 | BPF_MEM))
      printf("stx16 r%d r%d %d", dst_reg, src_reg, off);
    else if (opcode == (BPF_STX | BPF_SIZE_32 | BPF_MEM))
      printf("stx32 r%d r%d %d", dst_reg, src_reg, off);
    else if (opcode == (BPF_STX | BPF_SIZE_64 | BPF_MEM))
      printf("stx64 r%d r%d %d", dst_reg, src_reg, off);

    /* BPF_STX_XADD */
    else if (opcode == (BPF_STX | BPF_SIZE_8 | BPF_XADD))
      printf("stxx8 r%d r%d %d", dst_reg, src_reg, off);
    else if (opcode == (BPF_STX | BPF_SIZE_16 | BPF_XADD))
      printf("stxx16 r%d r%d %d", dst_reg, src_reg, off);
    else if (opcode == (BPF_STX | BPF_SIZE_32 | BPF_XADD))
      printf("stxx32 r%d r%d %d", dst_reg, src_reg, off);
    else if (opcode == (BPF_STX | BPF_SIZE_64 | BPF_XADD))
    ...
    */
    /* BPF_JMP_AL */
    // jal imm
    else if (opcode == (BPF_JMP32 | BPF_JA))
      printf("jal %d", imm);
