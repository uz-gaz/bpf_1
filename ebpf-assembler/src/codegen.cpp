...
static inline Int64 val64(Nat i, Nat j) {
  return imm(i, j).val;
}
...
void codegen(Str out_fname) {
  ...
  for (Nat i=0; i<ASIZE; ++i) {
    ...
    case sdiv:
      if (ty2 == Immediate)
        prog[insns++] = BPF_ALU64_S_IMM(BPF_DIV, reg_(FST, i), val(SND, i));
      else
        prog[insns++] = BPF_ALU64_S_REG(BPF_DIV, reg_(FST, i), reg_(SND, i));
    END
    ...
    case smod:
      if (ty2 == Immediate)
        prog[insns++] = BPF_ALU64_S_IMM(BPF_MOD, reg_(FST, i), val(SND, i));
      else
        prog[insns++] = BPF_ALU64_S_REG(BPF_MOD, reg_(FST, i), reg_(SND, i));
    END
    ...
    case movsx8:
      if (ty2 == Immediate)
        prog[insns++] = BPF_MOVSX64_IMM(8, reg_(FST, i), val(SND, i));
      else
        prog[insns++] = BPF_MOVSX64_REG(8, reg_(FST, i), reg_(SND, i));
    END

    /* movsx16 dst src|imm */
    case movsx16:
      if (ty2 == Immediate)
        prog[insns++] = BPF_MOVSX64_IMM(16, reg_(FST, i), val(SND, i));
      else
        prog[insns++] = BPF_MOVSX64_REG(16, reg_(FST, i), reg_(SND, i));
    END

    /* movsx32 dst src|imm */
    case movsx32:
      if (ty2 == Immediate)
        prog[insns++] = BPF_MOVSX64_IMM(32, reg_(FST, i), val(SND, i));
      else
        prog[insns++] = BPF_MOVSX64_REG(32, reg_(FST, i), reg_(SND, i));
    END
    ...
    case sdiv32:
      if (ty2 == Immediate)
        prog[insns++] = BPF_ALU32_S_IMM(BPF_DIV, reg_(FST, i), val(SND, i));
      else
        prog[insns++] = BPF_ALU32_S_REG(BPF_DIV, reg_(FST, i), reg_(SND, i));
    END
    ...
    case smod32:
      if (ty2 == Immediate)
        prog[insns++] = BPF_ALU32_S_IMM(BPF_MOD, reg_(FST, i), val(SND, i));
      else
        prog[insns++] = BPF_ALU32_S_REG(BPF_MOD, reg_(FST, i), reg_(SND, i));
    END
    ...
    /* mov32sx8 dst src|imm */
    case mov32sx8:
      if (ty2 == Immediate)
        prog[insns++] = BPF_MOVSX32_IMM(8, reg_(FST, i), val(SND, i));
      else
        prog[insns++] = BPF_MOVSX32_REG(8, reg_(FST, i), reg_(SND, i));
    END

    /* mov32sx16 dst src|imm */
    case mov32sx16:
      if (ty2 == Immediate)
        prog[insns++] = BPF_MOVSX32_IMM(16, reg_(FST, i), val(SND, i));
      else
        prog[insns++] = BPF_MOVSX32_REG(16, reg_(FST, i), reg_(SND, i));
    END
    ...
    */
    ...
    /* bswap16 dst */
    case bswap16:
      prog[insns++] = BPF_BYTESWAP(reg_(FST, i), 16);
    END

    /* bswap32 dst */
    case bswap32:
      prog[insns++] = BPF_BYTESWAP(reg_(FST, i), 32);
    END

    /* bswap64 dst */
    case bswap64:
      prog[insns++] = BPF_BYTESWAP(reg_(FST, i), 64);
    END
    ...
    case addx64:
      prog[insns++] =
        BPF_ATOMIC_OP(BPF_SIZE_64, BPF_ADD, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END
    ...
    case andx64:
      prog[insns++] =
        BPF_ATOMIC_OP(BPF_SIZE_64, BPF_AND, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END
    ...
    case orx64:
      prog[insns++] =
        BPF_ATOMIC_OP(BPF_SIZE_64, BPF_OR, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END
    ...
    case xorx64:
      prog[insns++] =
        BPF_ATOMIC_OP(BPF_SIZE_64, BPF_XOR, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END
    ...
    case addfx64:
      prog[insns++] =
        BPF_ATOMIC_OP(BPF_SIZE_64, BPF_ADD | BPF_FETCH, reg_(FST, i), reg_(SND, i),
                                               (__s16)val(TRD, i));
    END
    ...
    case andfx64:
      prog[insns++] =
        BPF_ATOMIC_OP(BPF_SIZE_64, BPF_AND | BPF_FETCH, reg_(FST, i), reg_(SND, i),
                                               (__s16)val(TRD, i));
    END
    ...
    case orfx64:
      prog[insns++] =
        BPF_ATOMIC_OP(BPF_SIZE_64, BPF_OR | BPF_FETCH, reg_(FST, i), reg_(SND, i),
                                              (__s16)val(TRD, i));
    END
    ...
    case xorfx64:
      prog[insns++] =
        BPF_ATOMIC_OP(BPF_SIZE_64, BPF_XOR | BPF_FETCH, reg_(FST, i), reg_(SND, i),
                                               (__s16)val(TRD, i));
    END
    ...
    /* xchgx64 dst src off */
    case xchgx64:
      prog[insns++] = BPF_ATOMIC_OP(BPF_SIZE_64, BPF_XCHG, reg_(FST, i), reg_(SND, i),
                                                  (__s16)val(TRD, i));
    END
    ...
    case cmpxchgx64:
      prog[insns++] = BPF_ATOMIC_OP(BPF_SIZE_64, BPF_CMPXCHG, reg_(FST, i), reg_(SND, i),
                                                     (__s16)val(TRD, i));
    END
    ...
    /* ld64 dst imm */
    case ld64:
      prog[insns++] = BPF_LD_IMM64_RAW_1(reg_(FST, i), 0, val64(SND, i));
      prog[insns++] = BPF_LD_IMM64_RAW_2(reg_(FST, i), 0, val64(SND, i));
    END
    ...
    /* ldabs64 imm */
    case ldabs64:
      prog[insns++] = BPF_LD_ABS(BPF_SIZE_64, val(FST, i));
    END
    ...
    case ldind64:
      prog[insns++] =
        BPF_LD_IND(BPF_SIZE_64, reg_(FST, i), val(SND, i));
    END

    /* ldx8 dst src off */
    case ldx8:
      prog[insns++] =
        BPF_LDX_MEM(BPF_SIZE_8, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END

    /* ldx16 dst src off */
    case ldx16:
      prog[insns++] =
        BPF_LDX_MEM(BPF_SIZE_16, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END

    /* ldx32 dst src off */
    case ldx32:
      prog[insns++] =
        BPF_LDX_MEM(BPF_SIZE_32, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END

    /* ldx64 dst src off */
    case ldx64:
      prog[insns++] =
        BPF_LDX_MEM(BPF_SIZE_64, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END

    /* ldxs8 dst src off */
    case ldxs8:
      prog[insns++] =
        BPF_LDX_MEMSX(BPF_SIZE_8, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END

    /* ldxs16 dst src off */
    case ldxs16:
      prog[insns++] =
        BPF_LDX_MEMSX(BPF_SIZE_16, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END

    /* ldxs32 dst src off */
    case ldxs32:
      prog[insns++] =
        BPF_LDX_MEMSX(BPF_SIZE_32, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END

    /* st8 dst off imm */
    case st8:
      prog[insns++] =
        BPF_ST_MEM(BPF_SIZE_8, reg_(FST, i), (__s16)val(SND, i), val(TRD, i));
    END

    /* st16 dst off imm */
    case st16:
      prog[insns++] =
        BPF_ST_MEM(BPF_SIZE_16, reg_(FST, i), (__s16)val(SND, i), val(TRD, i));
    END

    /* st32 dst off imm */
    case st32:
      prog[insns++] =
        BPF_ST_MEM(BPF_SIZE_32, reg_(FST, i), (__s16)val(SND, i), val(TRD, i));
    END

    /* st64 dst off imm */
    case st64:
      prog[insns++] =
        BPF_ST_MEM(BPF_SIZE_64, reg_(FST, i), (__s16)val(SND, i), val(TRD, i));
    END

    /* stx8 dst src off */
    case stx8:
      prog[insns++] =
        BPF_STX_MEM(BPF_SIZE_8, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END

    /* stx16 dst src off */
    case stx16:
      prog[insns++] =
        BPF_STX_MEM(BPF_SIZE_16, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END

    /* stx32 dst src off */
    case stx32:
      prog[insns++] =
        BPF_STX_MEM(BPF_SIZE_32, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END

    /* stx64 dst src off */
    case stx64:
      prog[insns++] =
        BPF_STX_MEM(BPF_SIZE_64, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END
    ...
    case stxx64:
      prog[insns++] =
        BPF_STX_XADD(BPF_SIZE_64, reg_(FST, i), reg_(SND, i), (__s16)val(TRD, i));
    END
    ...
    /* ja off */
    case ja:
      //std::printf("ja %d - %ld - 1 (%d)\n", val(FST, i), curr_ins, __s16(val(FST, i)));
    ...
    /* call imm */
    case call:
      prog[insns++] = BPF_CALL_AUX(val(FST, i));
    END
    ...
    /* jal imm */
    case jal:
      //std::printf("jal %d - %ld - 1 (%d)\n", val(FST, i), curr_ins, __s32(val(FST, i)));
      prog[insns++] = BPF_JMP_AL(__s32(val(FST, i)));
    END
...
void codegen_str(Str out_fname, Str struct_name) {
  ...
  for (Nat i=0; i<ASIZE; ++i) {
    ...
    case movsx8:
      if (ty2 == Immediate)
        c_code += "BPF_MOVSX64_IMM(8, " + reg_str(FST, i)  + ", "
                                        + STR(val(SND, i)) + "),\n";
      else
        c_code += "BPF_MOVSX64_REG(8, " + reg_str(FST, i) + ", "
                                        + reg_str(SND, i) + "),\n";
    END
    ...
    case movsx16:
      if (ty2 == Immediate)
        c_code += "BPF_MOVSX64_IMM(16, " + reg_str(FST, i)  + ", "
                                        + STR(val(SND, i)) + "),\n";
      else
        c_code += "BPF_MOVSX64_REG(16, " + reg_str(FST, i) + ", "
                                        + reg_str(SND, i) + "),\n";
    END
    ...
    case movsx32:
      if (ty2 == Immediate)
        c_code += "BPF_MOVSX64_IMM(32, " + reg_str(FST, i)  + ", "
                                        + STR(val(SND, i)) + "),\n";
      else
        c_code += "BPF_MOVSX64_REG(32, " + reg_str(FST, i) + ", "
                                        + reg_str(SND, i) + "),\n";
    END
    ...
    case mov32sx8:
      if (ty2 == Immediate)
        c_code += "BPF_MOVSX32_IMM(8, " + reg_str(FST, i)  + ", "
                                        + STR(val(SND, i)) + "),\n";
      else
        c_code += "BPF_MOVSX32_REG(8, " + reg_str(FST, i) + ", "
                                        + reg_str(SND, i) + "),\n";
    END
    ...
    case mov32sx16:
      if (ty2 == Immediate)
        c_code += "BPF_MOVSX32_IMM(16, " + reg_str(FST, i)  + ", "
                                         + STR(val(SND, i)) + "),\n";
      else
        c_code += "BPF_MOVSX32_REG(16, " + reg_str(FST, i) + ", "
                                         + reg_str(SND, i) + "),\n";
    END
    ...
    /* bswap16 dst src|imm */
    case bswap16:
       c_code += "BPF_BYTESWAP(" + reg_str(FST, i) + ", 16),\n";
    END

    /* bswap32 dst src|imm */
    case bswap32:
       c_code += "BPF_BYTESWAP(" + reg_str(FST, i) + ", 32),\n";
    END

    /* bswap64 dst src|imm */
    case bswap64:
       c_code += "BPF_BYTESWAP(" + reg_str(FST, i) + ", 64),\n";
    END
    ...
    /* addx64 dst src off */
    case addx64:
       c_code += "BPF_ATOMIC_OP(BPF_SIZE_64, BPF_ADD, "
              +  reg_str(FST, i)         + ", "
              +  reg_str(SND, i)         + ", "
              +  STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* andx64 dst src off */
    case andx64:
       c_code += "BPF_ATOMIC_OP(BPF_SIZE_64, BPF_AND, "
              +  reg_str(FST, i)         + ", "
              +  reg_str(SND, i)         + ", "
              +  STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* orx64 dst src off */
    case orx64:
       c_code += "BPF_ATOMIC_OP(BPF_SIZE_64, BPF_OR, "
              +  reg_str(FST, i)         + ", "
              +  reg_str(SND, i)         + ", "
              +  STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* xorx64 dst src off */
    case xorx64:
       c_code += "BPF_ATOMIC_OP(BPF_SIZE_64, BPF_XOR, "
              +  reg_str(FST, i)         + ", "
              +  reg_str(SND, i)         + ", "
              +  STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* addfx64 dst src off */
    case addfx64:
       c_code += "BPF_ATOMIC_OP(BPF_SIZE_64, BPF_ADD | BPF_FETCH, "
              +  reg_str(FST, i)         + ", "
              +  reg_str(SND, i)         + ", "
              +  STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* andfx64 dst src off */
    case andfx64:
       c_code += "BPF_ATOMIC_OP(BPF_SIZE_64, BPF_AND | BPF_FETCH, "
              +  reg_str(FST, i)         + ", "
              +  reg_str(SND, i)         + ", "
              +  STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* orfx64 dst src off */
    case orfx64:
       c_code += "BPF_ATOMIC_OP(BPF_SIZE_64, BPF_OR | BPF_FETCH, "
              +  reg_str(FST, i)         + ", "
              +  reg_str(SND, i)         + ", "
              +  STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* xorfx64 dst src off */
    case xorfx64:
       c_code += "BPF_ATOMIC_OP(BPF_SIZE_64, BPF_XOR | BPF_FETCH, "
              +  reg_str(FST, i)         + ", "
              +  reg_str(SND, i)         + ", "
              +  STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* xchgx64 dst src off */
    case xchgx64:
       c_code += "BPF_ATOMIC_OP(BPF_SIZE_64, BPF_XCHG, "
              +  reg_str(FST, i)         + ", "
              +  reg_str(SND, i)         + ", "
              +  STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* cmpxchgx64 dst src off */
    case cmpxchgx64:
       c_code += "BPF_ATOMIC_OP(BPF_SIZE_64, BPF_CMPXCHG, "
              +  reg_str(FST, i)         + ", "
              +  reg_str(SND, i)         + ", "
              +  STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* ldabs64 imm  */
    case ldabs64:
      c_code += "BPF_LD_ABS(BPF_SIZE_64, " + STR(val(FST, i)) + "),\n";
    END
    ...
    /* ldind64 src imm  */
    case ldind64:
       c_code += "BPF_LD_IND(BPF_SIZE_64, " + reg_str(FST, i)  + ", "
                                   + STR(val(SND, i)) + "),\n";
    END

    /* ldx8 dst src off */
    case ldx8:
       c_code += "BPF_LDX_MEM(BPF_SIZE_8, " + reg_str(FST, i)         + ", "
                                   + reg_str(SND, i)         + ", "
                                   + STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* ldx16 dst src off */
    case ldx16:
       c_code += "BPF_LDX_MEM(BPF_SIZE_16, " + reg_str(FST, i)         + ", "
                                    + reg_str(SND, i)         + ", "
                                    + STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* ldx32 dst src off */
    case ldx32:
       c_code += "BPF_LDX_MEM(BPF_SIZE_32, " + reg_str(FST, i)         + ", "
                                    + reg_str(SND, i)         + ", "
                                    + STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* ldx64 dst src off */
    case ldx64:
       c_code += "BPF_LDX_MEM(BPF_SIZE_64, " + reg_str(FST, i)         + ", "
                                    + reg_str(SND, i)         + ", "
                                    + STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* ldxs8 dst src off */
    case ldxs8:
       c_code += "BPF_LDX_MEMSX(BPF_SIZE_8, " + reg_str(FST, i)         + ", "
                                     + reg_str(SND, i)         + ", "
                                     + STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* ldx16 dst src off */
    case ldxs16:
       c_code += "BPF_LDX_MEMSX(BPF_SIZE_16, " + reg_str(FST, i)         + ", "
                                    + reg_str(SND, i)         + ", "
                                    + STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* ldx32 dst src off */
    case ldxs32:
       c_code += "BPF_LDX_MEMSX(BPF_SIZE_32, " + reg_str(FST, i)         + ", "
                                      + reg_str(SND, i)         + ", "
                                      + STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* st8 dst off imm  */
    case st8:
       c_code += "BPF_ST_MEM(BPF_SIZE_8, " + reg_str(FST, i)         + ", "
                                  + STR((__s16)val(SND, i)) + ", "
                                  + STR(val(TRD, i))        + "),\n";
    END
    ...
    /* st16 dst off imm   */
    case st16:
       c_code += "BPF_ST_MEM(BPF_SIZE_16, " + reg_str(FST, i)         + ", "
                                   + STR((__s16)val(SND, i)) + ", "
                                   + STR(val(TRD, i))        + "),\n";
    END
    ...
    /* st32 dst off imm   */
    case st32:
       c_code += "BPF_ST_MEM(BPF_SIZE_32, " + reg_str(FST, i)         + ", "
                                   + STR((__s16)val(SND, i)) + ", "
                                   + STR(val(TRD, i))        + "),\n";
    END
    ...
    /* st64 dst off imm   */
    case st64:
       c_code += "BPF_ST_MEM(BPF_SIZE_64, " + reg_str(FST, i)         + ", "
                                   + STR((__s16)val(SND, i)) + ", "
                                   + STR(val(TRD, i))        + "),\n";
    END
    ...
    /* stx8 dst src off */
    case stx8:
       c_code += "BPF_STX_MEM(BPF_SIZE_8, " + reg_str(FST, i)         + ", "
                                   + reg_str(SND, i)         + ", "
                                   + STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* stx16 dst src off */
    case stx16:
       c_code += "BPF_STX_MEM(BPF_SIZE_16, " + reg_str(FST, i)         + ", "
                                    + reg_str(SND, i)         + ", "
                                    + STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* stx32 dst src off */
    case stx32:
       c_code += "BPF_STX_MEM(BPF_SIZE_32, " + reg_str(FST, i)         + ", "
                                    + reg_str(SND, i)         + ", "
                                    + STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* stx64 dst src off */
    case stx64:
       c_code += "BPF_STX_MEM(BPF_SIZE_64, " + reg_str(FST, i)         + ", "
                                    + reg_str(SND, i)         + ", "
                                    + STR((__s16)val(TRD, i)) + "),\n";
    END
    ...
    /* stxx64 dst src off */
    case stxx64:
       c_code += "BPF_STX_XADD(BPF_SIZE_64, " + reg_str(FST, i)         + ", "
                                     + reg_str(SND, i)         + ", "
                                     + STR((__s16)val(TRD, i)) + "),\n";
    END
